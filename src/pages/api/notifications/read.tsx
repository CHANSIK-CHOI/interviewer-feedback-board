...
export default async function handler(
  ...
  try {
    const body = req.body ?? {};
    const notiIds: NotificationRow["id"][] | undefined = body.ids;
    if (!Array.isArray(notiIds) || notiIds.length <= 0) {
      return res.status(405).json({ data: null, error: "전달된 알림의 id가 없습니다." });
    }

    const ids = Array.from(
      new Set(
        notiIds
          .filter((id): id is NotificationRow["id"] => typeof id === "string")
          .map((id) => id.trim())
          .filter(Boolean)
      )
    );
